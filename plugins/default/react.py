...
if answ[1] == 'реакт':
	if (answ_text == ''):
		apisay('Введи тег блять',toho,torep)
	else:
...
if answ[1] == 'преакт':
	if (answ_text == ''):
		...
		count = requests.get('http://pornreactor.cc/search/'+answ_text.replace(' ','+')).text
		count = re.split('<a href=\'/search/',count)
		count = count[len(count)-2]
		count = count.split('>')[1].replace('</a','')
		count = random.randint(0,int(count))
		parse = requests.get('http://obxxe3tsmvqwg5dpoixggyy.cmle.ru/search?q='+answ_text.replace(' ','+')+'/'+str(count)).text
		strings = ['class="prettyPhotoLink" rel="prettyPhoto"><img src="', '" width="']
		try:
			parse = re.split(strings[0],parse)
			randint = random.randint(1,len(parse))
